...
import decred.memories.payload.response.AgendasResponse;
...
public class DcrdataAgendaServiceImpl implements DcrdataAgendaService {
	...
	public List<DecredMemoriesAgendaResponse> getAgendaByDate(String inputDate) throws ParseException {
	   ...
	   if(responseEntity.getBody() != null) {
		   ...
		   for(int i = 0; i < size; i++) {
			   Date start = response.get(i).getStartTime();
			   Date expire = response.get(i).getExpireTime();
